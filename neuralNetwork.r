...
optimizeNeuralNetworkHiddenLayer <- function(id)
{
  ...
  if (is.null(neuralNetwork.hiddenNodesOptimization[[id]]))
  {
    ...
    second_hidden_layer <- data.windowSize + 1
    for (i in 1:second_hidden_layer){
      for(j in 1:data.windowSize){
        if(i == 2) # 0 must not be set in hidden layer
        {
          neuralNetwork.hiddenLayersOptimization <<- c(j, i-1)
        }
        else
        {          
          neuralNetwork.hiddenLayersOptimization <<- c(j)
        }
        neuralNetwork.testResults.hiddenNodesOptimization[[id]] <<- testNeuralNetwork(getNeuralNetwork(id, hNodesOptimization = TRUE), id)
        ...
        {
          cat("Local optimum of number of nodes", c(j, i-1), nofill = TRUE)
          neuralNetwork.hiddenLayersOptimization <<- c(j, i-1)
